...
class CLaRaConfig(PretrainedConfig):
    ...
    def __init__(self,
        ...
        self.hybrid_adaptive_fusion = hybrid_adaptive_fusion
...
class CLaRa(PreTrainedModel):
    ...
    @staticmethod
    def _specific_token_ratio(text: str) -> float:
        """Estimate whether a query depends on exact lexical matching."""
        return specific_token_ratio(text)

    @staticmethod
    def _normalize_bm25_scores(scores: torch.Tensor) -> torch.Tensor:
        """Normalize BM25 scores per query while preserving zero rows."""
        return normalize_bm25_scores(scores)

    def _compute_bm25_scores(self, questions: List[str], documents: List[List[str]],
    ...
    def _compute_hybrid_alpha(self, questions: List[str], bm25_scores: torch.Tensor) -> torch.Tensor:
        """Return per-query latent-score weights for fixed or adaptive fusion."""
        return compute_hybrid_alpha(
            questions,
            bm25_scores,
            fixed_alpha=self.hybrid_alpha,
            adaptive=self.hybrid_adaptive_fusion,
            alpha_min=self.hybrid_alpha_min,
            alpha_max=self.hybrid_alpha_max,
        )

    def _fuse_retrieval_scores(self, latent_scores: torch.Tensor,
                               questions: Optional[List[str]],
                               documents: Optional[List[List[str]]]) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        """Fuse latent cosine scores with BM25 scores for hybrid retrieval."""
        return fuse_retrieval_scores(
            latent_scores,
            questions,
            documents,
    ...
    def _forward_stage2_batch(self, batch: Dict, stage2_mips: bool, stage2_retrieval_top_n: int) -> Tuple[torch.Tensor, Dict]:
        ...
        mse_loss = torch.tensor(0.0, device=self.decoder.device)
        bm25_scores = None
        hybrid_alpha = None
        query_reps = self._compr_query_reasoner_stage2(
        ...
        if stage2_mips:
            ...
            scores, bm25_scores, hybrid_alpha = self._fuse_retrieval_scores(
    ...
    def _forward_stage2_pretrain_batch(self, batch: Dict, stage2_mips: bool, stage2_retrieval_top_n: int) -> Tuple[torch.Tensor, Dict]:
        ...
        scores, _, _ = self._fuse_retrieval_scores(scores, batch.get("questions"), batch.get("docs"))
