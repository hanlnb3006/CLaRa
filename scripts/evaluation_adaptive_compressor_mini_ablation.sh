#!/bin/bash
#
# Lightweight ablation for inference-time Adaptive Compressor.
# Runs baseline CLaRa retrieval and CLaRa + query-aware memory-token gating.
#
...
ADAPTIVE_COMPRESSOR_TOP_K=${ADAPTIVE_COMPRESSOR_TOP_K:-3}
ADAPTIVE_COMPRESSOR_STRENGTH=${ADAPTIVE_COMPRESSOR_STRENGTH:-0.25}
ADAPTIVE_COMPRESSOR_TEMPERATURE=${ADAPTIVE_COMPRESSOR_TEMPERATURE:-0.10}
...
ADAPTIVE_ARGS="--adaptive_compressor"
ADAPTIVE_ARGS="$ADAPTIVE_ARGS --adaptive_compressor_top_k $ADAPTIVE_COMPRESSOR_TOP_K"
ADAPTIVE_ARGS="$ADAPTIVE_ARGS --adaptive_compressor_strength $ADAPTIVE_COMPRESSOR_STRENGTH"
ADAPTIVE_ARGS="$ADAPTIVE_ARGS --adaptive_compressor_temperature $ADAPTIVE_COMPRESSOR_TEMPERATURE"
...
echo "Running Adaptive Compressor retrieval..."
accelerate launch \
    --num_processes=$NUM_PROCESSES \
    --num_machines=1 \
    --mixed_precision=$MIXED_PRECISION \
...
echo "Adaptive Compressor mini ablation completed."
