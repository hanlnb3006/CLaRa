...
class DataLoader:
    ...
    @staticmethod
    def _resolve_eval_file(eval_data_root: Optional[str], stage_dir: str, legacy_stage_dir: str,
                           dataset: str, retrieval_type: str) -> str:
        """Resolve evaluation JSONL path from configurable or legacy roots."""
        filename = f"eval_processed_{retrieval_type}.jsonl"

        if eval_data_root:
            candidates = [
                os.path.join(eval_data_root, stage_dir, dataset, filename),
                os.path.join(eval_data_root, legacy_stage_dir, dataset, filename),
                os.path.join(eval_data_root, dataset, filename),
            ]
            for path in candidates:
                if os.path.exists(path):
                    return path
            raise FileNotFoundError(
                f"Could not find {filename} for dataset '{dataset}'. Tried: {candidates}"
            )

        return (
            "/mnt/conductor_data/data/compression_rag_data/generator_training_val_data/"
            f"{legacy_stage_dir}/{dataset}/{filename}"
        )
    
    @staticmethod
    def load_stage1_data(dataset: str, gold_retrieval: bool, eval_data_root: Optional[str] = None) -> List[Dict]:
        """Load stage1 evaluation data."""
        retrieval_type = "with_pos" if gold_retrieval else "no_pos"
        file_path = DataLoader._resolve_eval_file(
            eval_data_root,
            stage_dir="instruction_tuning_evaluation",
            legacy_stage_dir="stage1_eval",
            dataset=dataset,
    ...
    @staticmethod
    def load_stage2_data(dataset: str, gold_retrieval: bool, eval_data_root: Optional[str] = None) -> List[Dict]:
        """Load stage2 evaluation data."""
        retrieval_type = "with_pos" if gold_retrieval else "no_pos"
        file_path = DataLoader._resolve_eval_file(
            eval_data_root,
            stage_dir="end_to_end_evaluation",
            legacy_stage_dir="stage2_eval",
            dataset=dataset,
            retrieval_type=retrieval_type,
        )
...
class AcceleratedCLaRaInference:
    ...
    def __init__(self, model_path: str, training_stage: str = None, 
        ...
        self.model = CLaRa.from_pretrained(model_path, **model_kwargs)
...
def _format_float_token(value: Optional[float]) -> str:
...
def get_retrieval_run_suffix(args) -> str:
    """Build a suffix so baseline/fixed/adaptive retrieval runs do not overwrite each other."""
    if args.stage != "stage2" or not args.hybrid_retrieval:
        return ""
...
def limit_eval_samples(processed_data: List[Dict], max_eval_samples: Optional[int], dataset: str) -> List[Dict]:
    """Limit evaluation samples for smoke tests without changing full benchmark defaults."""
    if max_eval_samples is None:
        return processed_data
    limited_data = processed_data[:max_eval_samples]
    print(f"Using first {len(limited_data)} / {len(processed_data)} samples for {dataset}")
    return limited_data


def validate_hybrid_args(args):
    """Validate and normalize hybrid retrieval arguments."""
    if args.hybrid_adaptive_fusion and not args.hybrid_retrieval:
        args.hybrid_retrieval = True

    for name in ["hybrid_alpha", "hybrid_alpha_min", "hybrid_alpha_max"]:
        value = getattr(args, name, None)
        if value is not None and not 0.0 <= value <= 1.0:
            raise ValueError(f"{name} must be in [0, 1]")

    if args.bm25_k1 is not None and args.bm25_k1 <= 0:
        raise ValueError("bm25_k1 must be positive")
    if args.bm25_b is not None and not 0.0 <= args.bm25_b <= 1.0:
...
def main():
    ...
    parser.add_argument('--checkpoint_root', type=str,
                       default='/mnt/task_wrapper/user_output/artifacts/data/train_checkpoint',
                       help='Root directory for model checkpoints when model_path is a checkpoint name')
    parser.add_argument('--eval_data_root', type=str, default=None,
                       help='Root directory containing extracted evaluation JSONL files')
    parser.add_argument('--max_eval_samples', type=int, default=None,
                       help='Limit each dataset to the first N samples for smoke tests')
    ...
    parser.add_argument('--hybrid_retrieval', action='store_true', default=None,
                       help='Enable BM25 + latent hybrid retrieval for stage2 inference')
    parser.add_argument('--hybrid_alpha', type=float, default=None,
                       help='Fixed latent-score weight for hybrid retrieval')
    parser.add_argument('--hybrid_adaptive_fusion', action='store_true', default=None,
                       help='Adapt latent/BM25 fusion weight per query')
    parser.add_argument('--hybrid_alpha_min', type=float, default=None,
                       help='Minimum latent-score weight for adaptive fusion')
    parser.add_argument('--hybrid_alpha_max', type=float, default=None,
    ...
    parser.add_argument('--compr_base_model_name', type=str, default=None,
                       help='Override compressor base model path or Hugging Face id')
    parser.add_argument('--compr_model_name', type=str, default=None,
                       help='Override compressor model path or Hugging Face id')
    parser.add_argument('--quantization', type=str, default=None, choices=['no', 'int4', 'int8'],
                       help='Override model quantization mode')
    parser.add_argument('--device_map', type=str, default=None,
                       help='Override Transformers device_map, e.g. auto')
    parser.add_argument('--attn_implementation', type=str, default=None,
                       help='Override attention implementation passed to Transformers')
    ...
    run_suffix = get_eval_run_suffix(args)
    ...
    for dataset in datasets_list:
        ...
        if args.stage in ['stage1', 'stage1_2']:
            processed_data = DataLoader.load_stage1_data(dataset, args.gold_retrieval, args.eval_data_root)
        elif args.stage == 'stage2':
            processed_data = DataLoader.load_stage2_data(dataset, args.gold_retrieval, args.eval_data_root)
        ...
        processed_data = limit_eval_samples(processed_data, args.max_eval_samples, dataset)
        ...
        if os.path.isabs(args.model_path) or os.path.exists(args.model_path):
            ...
            model_path = os.path.join(args.checkpoint_root, args.model_path)
        ...
        if args.efficient_count and inference_engine.accelerator.is_main_process:
            ...
            eff_output_path = os.path.join(
                model_path,
                f"efficiency_{dataset}_{args.stage}_{args.gold_retrieval}_{args.generation_top_k}{run_suffix}.json"
            )
        ...
        if inference_engine.accelerator.is_main_process:
            ...
            output_path = os.path.join(
                model_path,
                f"{dataset}_{args.stage}_{args.gold_retrieval}_{args.generation_top_k}{run_suffix}.jsonl"
            )
    ...
    if len(all_results_metrics) > 0:
        metrics_path = os.path.join(
            model_path,
            f"results_metrics_{args.stage}_{args.gold_retrieval}_{args.generation_top_k}{run_suffix}.json"
        )
